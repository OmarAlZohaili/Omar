...
//exercise1
	function crystalGazer(childnr, job, partnerName, geographic){
		...
		try {
		if(childnr == '' || job == ''|| partnerName == ''|| geographic== '') return "wrong inPut";
        if(typeof childnr != 'number') return "wrong input for childnr number";
        if(typeof job != 'string') return "job is not a string";
        if(typeof partnerName != 'string') return "partnerName is not a string";
        if(typeof geographic != 'string') return "geographic is not a string";
	    }
	    catch(err) {
	        return err;
	    }
